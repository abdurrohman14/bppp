                    ...
                    <tbody>
                        @foreach($pakanMasuk as $key => $item)
                            <tr>
                                ...
                                <td class="text-start">{{ $item->pakan->nama ?? 'Pakan Tidak Ditemukan' }}</td>
                                <td class="text-start">{{ $item->tanggal_masuk }}</td>
                                <td class="text-start">{{ $item->jumlah_masuk }}</td>
                                <td style="padding-left: 12px;">
                                    <div class="d-flex gap-2">
                                        <a href="{{ route('edit.petugas.PakanMasuk', $item->id) }}" class="btn btn-warning btn-sm text-white" style="width: 80px;">Edit</a>
                                        <form action="{{ route('destroy.petugas.PakanMasuk', $item->id) }}" method="POST" style="width: 80px;">
                                            @csrf
                                            @method('DELETE')
